...
import (
	...
	"github.com/VladVes/go-tinker/v2/internal/store"
...
func Run() {
	...
	viewsEngine := html.New("./internal/web/templates", ".tmpl")
	...
	app := fiber.New(fiber.Config{
		...
		// **`ReadBufferSize`**
		// Предположим, клиент отправляет запрос с заголовком `Authorization`, содержащим JWT‑токен длиной 8 КБ,
		// и ещё несколькими заголовками общим размером 2 КБ:
	...
	app.Get("/list_tmpl", func(c *fiber.Ctx) error {
		...
		if _, err := os.Stat("./internal/web/templates/list.tmpl"); os.IsNotExist(err) {
			return c.Status(fiber.StatusNotFound).SendString("Template file not found: " + err.Error())
		}
		return c.Render("list", store.ListExample)
	})
	//=====================================================================================================
	...
	app.Get("/likes/:postId", func(c *fiber.Ctx) error {
		...
		likes, ok := store.PostLikes[postId]
		if !ok {
			logrus.WithFields(logrus.Fields{
				"postId": postId,
	...
	app.Post("/likes/:postId", func(c *fiber.Ctx) error {
		...
		store.PostLikes[postId]++
		likes := store.PostLikes[postId]
	...
	// потому как store.Users это map который является ссылочным типом
